...
mathematical question (probes cannot settle computability).
THE BEAVER'S CORRECTION (Stage 313): the
diagonal plateau is RETRACTED — the n=9 beaver pair needs minimal
peak > 59 (excess ≥ 51 vs 23 at n=8); the earlier n=10 sweep's skip
heuristics hid the mountain class. Peaks GROW (slowly); the
two-curves gap survives at four orders of magnitude (≥51 vs 15,044);
C12 is alive but harder than the plateau suggested. THE DEV TOWER IS COFINAL (Stage 307,
...
seeds — eternal engines exist EXACTLY at pure-C slots (six engines,
five climbers, one pump); the purity law registered; the corridor
phase looks like a Catalan-indexed CATALOG. THE MILL FAMILY LAW (Stage 287,
...
`sc_forced_family_line`): total order extracted into the generic
pipeline; the swapmill's line is a one-line instance. THE SECOND
TRICHOTOMY CORE (Stage 271,
...
`sc_swap_eternal`/`sc_swap_unbounded`): the second engine runs
forever — a fifteen-leaf pure seed with unbounded reachable set; two
complete engine cores now pinned. THE RIDDEN REVOLUTION (Stage 267,
...
LAW (Stage 262, `sc_cold_law`, ninth
wall): C-fires never mint C-redexes — the storm floor's cold half is
a theorem; C14's remaining debt is one lemma (the hot minting bound).
THE COLD FRAGMENT (Stage 261, `sc_descent_speed`):
...
embedded G-machines. THE TRICHOTOMY (Stage 250, `sc_mt5T_flat`): the
...
theorem. The corridor is now INESCAPABLE, not merely
