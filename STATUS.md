...
| 2 | `{S,B}` | **open; termination-based routes CLOSED (Stage 80: `SB_not_normalizing` — the full system has divergent terms, so no match-bound or any termination certificate can exist; the live routes are loop-freeness transformations or an unbounded well-founded measure). Stage 81 REDUCED the problem: a minimal cycle passes through a root redex (`sb_cycle_needs_root`), so acyclicity follows from ruling out root-redex cycles alone (`sb_acyclic_of_no_root_cycle`). Stage 82 split each root cycle's return path: another root step, or COLLAPSE TO ARGUMENT (`u v ⟶* v`) plus self-embedding (`sb_root_S_return`, `sb_root_B_return`) — collapse is the single named sub-target killing every non-root branch.** No counting measure is monotone (`no_monotone_counting_measure`). No cycle under **any** strategy up to 8 leaves within a 30-leaf cap, cap-insensitive to 120 (`onCycleAny`); the cap is not liftable by brute force. **τ strictly drops on every B-reduction and every τ-light S-reduction, so the τ-light fragment is ACYCLIC** (`sbLight_acyclic`) — hence any cycle must fire an S-reduction duplicating a τ-**heavy** argument (`sbCycle_needs_heavy_S`) |
| 3 | `{S,C}` | **open; termination-based routes likewise CLOSED (`SC_not_normalizing`); same Stage 81 root-cycle reduction (`sc_acyclic_of_no_root_cycle`). Structurally UNLIKE rung 2.** `C x y z → x z y` has the same `leafCount` delta as `B` (−1), so no counting measure separates them — but τ does: B always lowers τ, **C can raise it** (`tauSC_C_red`, delta `τ(z)−τ(y)−8`), because permuting moves a heavy argument into a lighter position. So the light fragment needs **two** conditions (`scLight_acyclic`). Censused: no cycle up to 6 leaves under any strategy within a 30-leaf cap |
