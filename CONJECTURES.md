...
### Stage 287: THE MILL FAMILY LAW — both motifs are families

`sc_millfam_law` (axiom-free): the mill's turnover is slot-parametric too — `K_z`
turns over with layer `z (C M)` for any `z`, and `scMillFamK (C C) = scMillK` by
`rfl`. The two driver motifs that account for every engine the census ever surfaced
are now BOTH one-law families. C13's mature form: the corridor phase is generated by
two slot-parametric cores (the swap driver and the mill core), each with its layer
grammar determined by its slot, each with junk that carries its own blueprint, each
yielding the full portrait by the generic pipeline. The enumeration of {S,C}'s
